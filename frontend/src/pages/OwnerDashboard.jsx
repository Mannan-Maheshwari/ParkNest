    ...
    <main className="min-h-[calc(100vh-64px)] bg-slate-50">
      ...
      <Modal open={modal} title={editing ? "Edit parking space" : "Add parking space"} onClose={() => setModal(false)}>
        <form onSubmit={save} className="space-y-4">
          {["name","address","description"].map((field) => <label key={field} className="block text-sm font-semibold text-slate-700">{field === "name" ? "Parking name" : field === "address" ? "Address" : "Description"}{field === "description" ? <textarea rows="3" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500" value={form[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })} /> : <input required className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500" value={form[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })} />}</label>)}
          <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">Price/hour<input required type="number" min="0" className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} /></label><label className="text-sm font-semibold">Total spots<input required type="number" min="1" className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3" value={form.totalSpots} onChange={(e) => setForm({ ...form, totalSpots: e.target.value })} /></label></div>
          {!editing && <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">Latitude<input required type="number" step="any" className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3" value={form.lat} onChange={(e) => setForm({ ...form, lat: e.target.value })} /></label><label className="text-sm font-semibold">Longitude<input required type="number" step="any" className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3" value={form.lng} onChange={(e) => setForm({ ...form, lng: e.target.value })} /></label></div>}
